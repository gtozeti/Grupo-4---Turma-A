...
public class teste004 {
    ...
    public static void analiseArquivo(String nomeArquivo) throws Exception {
        ...
        while (cond) {
            ...
            // Ao armazenar cada palavra da frase de cada linha, se inicia a verificação de
            // cada palavra armazenada no vetor
            for (int i = 0; i < frase.length; i++) {

                // Por conta da ocorrência de gerar palavras vazias com o método split(), foi
                // incluída essa verificação
                if (!frase[i].isEmpty()) {
    ...
    public static boolean verificaDict(String palavra) {
        ...
        // Manipulação da "setorização" do vetor para encontrar se o elemento está ou
        // não dentro do vetor
        while (inicio <= fim) {
            meio = (inicio + fim) / 2;

            // Para obter um valor númerico para a questão de maior ou menor da String, foi
            // utilizado o método compareTo
